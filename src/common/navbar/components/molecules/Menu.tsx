'use client'
import { NLink } from '@/common/navbar/components/atoms/Link'
import type { MenuProps } from '@/common/navbar/types'
import { Bars3Icon, XMarkIcon } from '@heroicons/react/24/solid'
import clsx from 'clsx'
import { MouseEvent, useEffect, useRef, useState } from 'react'

const ANIMATION_MS = 500

/**
 * @description Renders menu component, including handling open/close state and animations.
 * @component
 * @param {MenuProps} props - Component properties.
 * @param {LinkItem[]} props.links - The links value.
 * @param {string} props.links[].text - The text value.
 * @param {string} props.links[].href - The URL of the link.
 * @param {string} props.links[].icon - The icon of the link.
 * @param {number} props.links[].id - The ID of the link.
 * @param {() => void} props.links[].handleCloseOptions - The function to handle closing.
 * @param {string} props.menuId - The ID of the menu.
 * @returns Returns menu component with responsive behavior and animations.
 */
export const Menu = ({ links, menuId }: MenuProps) => {
  const closeTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  const [isOpen, setIsOpen] = useState(false)
  const [isVisible, setIsVisible] = useState(false)
  const handleOpenOptions = () => {
    if (isOpen) return
    if (closeTimeoutRef.current) {
      clearTimeout(closeTimeoutRef.current)
      closeTimeoutRef.current = null
    }
    setIsVisible(true)
    requestAnimationFrame(() => {
      setIsOpen(true)
    })
  }
  const handleCloseOptions = () => {
    if (!isOpen) return
    setIsOpen(false)
    closeTimeoutRef.current = setTimeout(() => {
      setIsVisible(false)
      closeTimeoutRef.current = null
    }, ANIMATION_MS)
  }
  useEffect(() => {
    return () => {
      if (closeTimeoutRef.current) {
        clearTimeout(closeTimeoutRef.current)
        closeTimeoutRef.current = null
      }
    }
  }, [])
  const handlePropagateOptions = (e: MouseEvent) => e.stopPropagation()
  return (
    <>
      <button
        type={'button'}
        aria-expanded={isOpen}
        aria-controls={menuId}
        aria-label={menuId}
        onClick={isOpen ? handleCloseOptions : handleOpenOptions}
        className={clsx(
          'horizontal',
          'size-10',
          'justify-center',
          'rounded-full',
          'shadow-lg',
          'bg-irongray',
          'm2x:size-10.25',
          'm3x:size-10.5',
          'md:size-11',
          'lg:hidden'
        )}
      >
        <span className={clsx('horizontal')}>
          {isOpen ? (
            <XMarkIcon
              aria-hidden={'true'}
              role={'img'}
              className={clsx('size-6.75', 'fill-primary', 'm1x:size-7', 'md:size-8')}
            />
          ) : (
            <Bars3Icon
              aria-hidden={'true'}
              role={'img'}
              className={clsx('size-6.75', 'fill-primary', 'm1x:size-7', 'md:size-8')}
            />
          )}
        </span>
      </button>
      {isVisible && (
        <div
          onClick={handleCloseOptions}
          className={clsx(
            'fixed',
            'inset-x-0',
            'top-21',
            'z-50',
            'h-dvh',
            'w-full',
            'transition-colors',
            'duration-150',
            isOpen ? 'bg-midnightgreen/60 backdrop-blur-sm' : 'bg-transparent backdrop-blur-none'
          )}
        >
          <div
            role={'dialog'}
            id={menuId}
            onClick={handlePropagateOptions}
            className={clsx(
              'w-full',
              'transform',
              'transition-all',
              'duration-500',
              'ease-in-out',
              'contain-paint',
              'will-change-transform',
              'bg-secondary',
              isOpen ? 'translate-y-0 opacity-100' : '-translate-y-3 opacity-0'
            )}
          >
            <div
              className={clsx(
                'grid',
                'grid-cols-3',
                'w-full',
                'gap-3.5',
                'px-5.25',
                'pt-7',
                'pb-13',
                'm1x:gap-4',
                'm1x:px-6',
                'm3x:gap-4.5',
                'md:grid-cols-4',
                'md:grid-rows-2',
                'md:gap-6',
                'md:px-13',
                'md:pt-12',
                'md:pb-16'
              )}
            >
              {links.map((item) => (
                <NLink
                  key={item.id}
                  text={item.text}
                  href={item.href}
                  icon={item.icon}
                  id={item.id}
                  handleCloseOptions={handleCloseOptions}
                />
              ))}
            </div>
          </div>
        </div>
      )}
    </>
  )
}
