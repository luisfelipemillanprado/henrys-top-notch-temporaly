...
/**
 * @description Renders a single step with an avatar, title, and supporting description.
 * @component
 * @param {object} props - Component properties.
 * @param {string} props.url - Image URL for the step.
 * @param {string} props.title - Step title text.
 * @param {string} props.description - Step description text.
 * @returns Returns a step block with image, heading, and description.
 */
export const Step = ({ url, title, description }: { url: string; title: string; description: string }) => {
  return (
    ...
    >
      <div
        className={clsx('vertical', 'w-full', 'items-center', 'gap-y-4.5', 'md:items-start', 'lg:gap-y-5')}
      >
        <Avatar url={url} background={true} />
